...
To illustrate our examples, we will use a sample data set containing a series of different measurements from replicated algal samples. You can, download the data set, [Algal_traits.csv](http://environmentalcomputing.net/wp-content/uploads/2016/05/Algal_traits.csv), and load it into R:
...
Below we spend some time outlining the two main *types* of function, *why* you should use functions, and then *how* they are constructed. 
...
For the first type, you'll often save the output in a variable and manipulate it further.  E.g. lets say we want to calculate the average of the variable `height` of the samples in the `algae` data. We can use the function `mean`:
...
By contrast, output from the second type of function does not need to be assigned to variable. Moreover, the function doesn't print anything to screen either. E.g.
...
So why is it so useful to divide your script into many separate, but cooperating, functions? Why not write one big, long script?

There are multiple ways in which writing functions can improve your coding. 
...
Consider some of the functions that you have already used within R.  For example, 
...
Functions quickly increase the ease of which you can read and interpret the code. It is immediately obvious what the `standard_error()` line of code is doing. Less obvious would be to use the following code and have to edit it extensively every time you wanted to calculate the standard error of a different variable.
...
Building on the idea of making code easier to read, we can write functions that organise our whole workflow. Your analysis script might end up looking something like this:
...
```
where all the functions like `fit_model_species` are one's you've written yourself
...
But what if you forget? There's a real danger that later you accidentally reuse the variable `n` or `var_x`, thinking they're something that they're not. And if they have non-specific names like `n` there's a real chance that could occur.

If instead you put the code above into a function like this, this danger disappears
...
Any variables created within a function get automatically cleaned up at the end of the function. So using functions leaves us with a nice clean workspace. Moroever, the environment within the function is much safer than the global env, because we're less likely to grab random variables from elsewhere.
...
Writing functions allows us to identify a series of smaller problems and solve these one by one. 

As a general rule, a good function does one thing well. If that *thing* is complicated, it may use a bunch of smaller functions to achieve it's outcome.
...
**function body**: The code between the `{}` is run every time the function is called. This is the code that is doing all the useful stuff and is called the function body.
...
Once defined we can call that object like we would any other function.  
...
Let's take a closer look at the function `mean`. Typing `?mean` into the console brings up the help. Note the structure

```{r, eval=F}
mean(x, trim = 0, na.rm = FALSE, ...)
```

The first argument `x` is our vector of numbers. To use the function we only need to specify something for `x`, e.g.
...
The first version makes it explicitly that the values in `x=algae$height`, i.e. from the functions perspective, `height` is `x`. The second version works because R takes the values of `height` and maps it onto `x`. It does this because it assigns the first unnamed argument in our function call onto the first unnamed argument in the function definition. So the following will also work
...
The example with `standard_error` is perhaps a bit trivia, but you can take this approach all over the place. 

### What's the `...` argument for?

The `...`, or ellipsis, element in the function definition allows for other arguments to be passed into the function, and passed onto to another function within the function. For example, in the definition of the function `standard_error_narm` we could instead write
