...
We read in the shapefile for the Australian coast and name it Aus in our environment. The argument `dsn` provides the path to the folde holding the shape files, while `layer` identifies which file we want. In this case "castauscd_r" holds the coastline data for Australia as a polygon). Page 9 of the the user guide that was downloaded with the shape file explains what all the other files are (the pdf file, 61395_GEODATA_COAST_100K_2004_User_Guide). 
