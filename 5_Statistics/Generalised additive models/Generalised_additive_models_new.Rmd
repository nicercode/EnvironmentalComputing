...
To see the fitted line, we can change the method used by ```geom_smooth```, specifying the model formula.
...
Using ```summary``` with the model object will give you ?????? and the variance explained. In this example, a pretty decent fit.
...
We'll now look at a quick real example - we'll just scratch the surface, and in a future tutorial we will look at it in more detail. We're going to look at some CO$_2$ data from Manua Loa (it's used elsewhere in this series). We will fit a couple GAMs to the data to try and pick apart the intra- and inter-annual trends.

First load the data - you can download it [here](http://environmentalcomputing.net/wp-content/uploads/2017/10/manua_loa_co2.csv).
```{r}
...
Note how the smooth term actually reduces to a 'normal' linear term here - that's the nice thing about penalised regression splines. But if we check the model, then we see something is amuck. 
```{r, results='hide', fig.keep='all'}
par(mfrow = c(2,2))
gam.check(CO2_time)
```

?????? describe what is wrong with the residual plots. 
...
Again, check the residual plots:
```{r}
par(mfrow = c(1,2))
plot(CO2_season_time)
```

Now looking at both smooth terms, we can see that the monthly smoother is picking up that monthly rise and fall of CO$_2$ - looking at the relative magnitudes, we can see how important it is to disintangle the components of the time series. Let's see how the model diagnostics look now:
```{r, results='hide', fig.keep='all'}
par(mfrow = c(2,2))
...
<br>
**Written**  ????? measures of model fit??

**Visual** ????

```{r}
ggplot(CO2_dat, aes(time, co2)) + geom_point() + geom_smooth(method = gam, formula = co2 ~ s(month, bs = 'cc', k = 12) + s(time))
```
<br><br>

### Further help
<br>
Type `?gam` to get the R help for this function. 
