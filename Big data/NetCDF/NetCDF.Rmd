<span style="color:red">How to open NetCDF files
========================================================
<br><br>
...
The worked example below will show you how to open, view and extract data from a NetCDF file in R. These files can have multiple dimensions (e.g., latitude, longitude, time, depth, altitude) as well as multiple variables (e.g., temperature, rainfall, habitat cover, chlorophyll-a), and hence require special formatting structure.

The sample data set used below is extracted from the Integrated Marine Observing System (IMOS) and is the sea surface temperature (SST) for the east coast of Australia on 6/12/15. While this example is aquatic, the application to terrestrial domains is the same. 

![](NetCDF_method_image.jpg)
<br><br>

### Install packages and load sample data file

Firstly, install and load the packages <font face="monospace">ncdf4</font>, needed to read, write, and create netCDF files and <font face="monospace">raster</font>, which provides functions for manipulating geographic data in 'raster' format (i.e., divided up into cells of equal size).
...
You can check the units of each variable with <font face="monospace">ncatt_get()</font>.
...
These data can be assigned to new objects with the function <font face="monospace">ncvar_get</font>.
...
You can check the dimensions of each with <font face="monospace">dim()</font>
```{r,eval=FALSE}
dim(temp)
dim(time)
dim(lat)
...
```{r}
#identify the x and y dimensions
...
This example shows you the basics of opening a NetCDF file and extracting the information. If you have multiple NetCDF files that you need to extract from, then you can simply set up the script above in a loop (see the 'looping' help module on moodle).
<br><br>
...
NetCDF files are commonly used in model prediction. For example you may want to predict the distribution of an animal given certain environmental conditions. You can make model predictions on a raster (see <font face="monospace">?predict</font> in the <font face="monospace">raster</font> package) or you can make predictions on the data frame (see <font face="monospace">?predict</font> in the <font face="monospace">stats</font> package). The model object for each predict function is typically a standard model object (lm, glm, gam etc.)
<br><br>
