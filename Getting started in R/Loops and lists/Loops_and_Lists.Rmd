...
Do you find yourself cutting and pasting R code a lot? Then using loops might help you streamline things! We often want to do repetitive tasks in Ecology: we may like to check whether multiple variables are correlated, or calculate descriptive statistics for multiple variables. Be aware that there are many packages in *R* with functions that will do all of the hard work for you (e.g. check out *reshape*, *plyr* and *dplyr*). However, there are always occasions when you need something specific done and only a list will do.
...
The other option is to make a blank vector or dataframe of known dimensions and then place the results into it directly. This is computationally much faster so it is good to get into the habitat of using this method.
...
In the above example, we iterated between columns in a dataframe, using *i* to call in the column we were interested in. A dataframe is a type of list, being a list of columns of the same length. However a list can be a list of any types of objects, for example we could make a list of a vector, a dataframe and a plot. We pull out items in the list using the double square brackets [[]]. 
