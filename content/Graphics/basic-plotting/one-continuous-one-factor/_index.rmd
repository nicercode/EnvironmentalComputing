...
Firstly, download the sample data file, [Prawns_MR.csv](/../wp-content/uploads/2016/05/Prawns_MR.csv), and import into R.
...
While commonly used, they are not so easy to create in the base functions in R. There are various ways to do this, but one is to use the `summarise` and `group_by` functions from the package [dplyr](https://cran.r-project.org/web/packages/dplyr/index.html) to calculate the means and measures of variation for each level of your categorical variable (see [Summarising data](/../summarising-data/)).

Here is some sample code to plot means +/- standard deviations. For more control over error bars, we recommend using the more advanced plotting options in the package ggplot2 (see [bar plots with error bars](/../plotting-with-ggplot-bar-plots-with-error-bars/)).
