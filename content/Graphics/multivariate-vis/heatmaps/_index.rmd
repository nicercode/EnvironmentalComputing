...
With multivariate data, we often have two data frames with 1) the counts per sample and 2) the factors that group samples. Download these two data files, [Bac.counts.csv](/../wp-content/uploads/2016/06/Bac.counts.csv) and [Bac.factors.csv](/../wp-content/uploads/2016/06/Bac.factors.csv), and import into R.
...
The basic function is `pheatmap`. Let's try it without special arguments, except that we will only look at the first 500 OTUs (they are arranged from highest to lowest total abundance already). The function `slice` in dplyr can take any subset of numbered rows (see [Subsetting data](/../subsetting-data/)). 
...
We will analyse the abundances of all OTUs with multivariate generalised linear models using the function `manyglm` in the package mvabund. The specifics of that analysis are not described further here (see the [Introduction to mvabund](/../introduction-to-mvabund/) for help with that package). Note that running `anova.manyglm` is quite slow.
