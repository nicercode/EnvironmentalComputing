...
I'll start by simulating a dataset which has an effect (a difference between the control and treatment), and we'll see if we can detect the effect. We'll do a simple [two sample t-test](/../independent-samples-t-test/) for illustration.
...
There are various packages which do power analysis in R. [pwr](https://cran.r-project.org/web/packages/pwr/index.html) does simple stuff up to `lm` and [simR](https://cran.r-project.org/web/packages/simr/index.html) does more complex [mixed models](/../mixed-models/) and [glms](/../generalised-linear-models/).
...
Download this pilot data, [Pilot.csv](/../wp-content/uploads/2016/09/Pilot.csv), import into R and change the modification variable into a factor (labelled as 1,2 and 3, but not an integer).
...
These count data with a random effect of site are best modelled with a [generalised linear mixed model](/../mixed-models-3/) using `glmer` from the package lme4.
