...
I'll start by simulating a dataset which has an effect (a difference between the control and treatment), and we'll see if we can detect the effect. We'll do a simple [two sample t-test](/../statistics/t-tests/two-sample-t-test/) for illustration.
...
There are various packages which do power analysis in R. [pwr](https://cran.r-project.org/web/packages/pwr/index.html) does simple stuff up to `lm` and [simR](https://cran.r-project.org/web/packages/simr/index.html) does more complex [mixed models](/../statistics/mixed-models/) and [glms](/../statistics/glms/).
...
If you are conducting an experiment with random factors, and you need to do a [mixed model](/../statistics/mixed-models/) and your power analysis needs to reflect that. This is much harder to do than the above examples, and we will need to use simulation methods. The package [simR](https://cran.r-project.org/web/packages/simr/index.html) is designed to do this.
...
These count data with a random effect of site are best modelled with a [generalised linear mixed model](/../statistics/mixed-models-3/) using `glmer` from the package lme4.
